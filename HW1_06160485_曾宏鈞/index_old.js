...
function drawNode()
{
    ...
    for (let i=0; i<drawNodeList.length; i++) {
        let x = drawNodeList[i][1] 
        let y = drawNodeList[i][2]
        graph.circle(x,y,30 )
        graph.text(drawNodeList[i][0],x-10,y+5)
...
function init_gui() {
    ...
    slider_range_from = createSlider(-100,50,-100)
    slider_range_from.position(btn_search.x, btn_search.y+btn_search.height+15)
    slider_range_to = createSlider(50,100,100)
...
class BinarySearchTree
{
    ...
    insert(data)
    {
        ...
        if (this.root === null) {
            this.root = newNode;
            drawNodeList.push([data,windowWidth/3,75, 0]) // data, x, y, depth
            newNode.x = windowWidth/3
            newNode.y = 75
        } else this.insertNode(this.root, newNode, windowWidth/3, 75, 0) //尋找新增節點的位置
    }
    // insertNode : 新增進樹的方法
    insertNode(node, newNode, x,y,depth)
    {
        
        let sh = 60/(depth+1) * 15
        if (sh>30) sh=29
        if (newNode.data < node.data){ // new小於root node，走左邊
            ...
            if(node.left === null) { //空的可以放
                ...
                //drawLineList.push([node.x,node.y, newNode.x, newNode.y])
                drawNodeList.push([newNode.data,x-sh,y+60,depth+1])
            } else { //有東西，往下一層遞迴找
                this.insertNode(node.left, newNode, x-sh, y+60, depth+1)
            }
            ...
            if(node.right === null) { //空的可以放
                ...
                this.insertNode(node.right, newNode, x+sh, y+60,depth+1)
            }
        }

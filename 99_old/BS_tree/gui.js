...
{
    bst.root = bst.prefix_infix(inp_otherfix.value(), inp_infix.value(),"origin:")
    update_traversal()
}
function construct_tree_post()
{
    bst.root = bst.postfix_infix(inp_otherfix.value(), inp_infix.value(),"origin:")
    update_traversal()
}
...
function update_traversal() {
    ...
    if (check_show_construct.checked()) {
        let msg=""
        bst.print_List.forEach(element => {
            msg += element + "<br>"
        });
        label_construct.html(msg)
    }
    else label_construct.html("")
    bst.print_List=[]
    draw_graph()
...
let inp_infix, inp_otherfix, label_construct
function init_gui() {
    ...
    check_show_construct = createCheckbox('show construct tree', true);
    label_construct = createP("")
    ...
    check_show_construct.position(windowWidth-inp_otherfix.width,btn_construct_levelOrder.y+btn_construct_levelOrder.height)
    label_construct.position(windowWidth-inp_otherfix.width,check_show_construct.y+check_show_construct.height)
    slider_change()
    update_traversal()
}
